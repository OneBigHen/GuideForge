...
| P04   | 04    | Real multimodal ingestion providers                                | active            | real CPU Docling/Whisper/ffmpeg smoke paths now produce citable regions, tables, timestamps, keyframes, quality reports, and runtime receipts; scanned OCR, figure/page-image extraction, CSV/XLSX citation coverage, VLM fallback, and the Pack golden corpus remain UNVERIFIED                                                                                   | run the remaining golden corpus on capable hardware and capture VLM/figure/OCR evidence |
...
current host proves real CPU Docling/Whisper/ffmpeg subpaths, but its scanned
OCR run exhausted swap, the figure/page-image path did not produce figure
objects, CSV/XLSX citation coverage warned, the local VLM could not load, and
the required golden corpus is still absent. Phase 05 is active but not verified
because the live DeepSeek key is absent. Phase 06 is verified narrowly on
