...
volumes: - guideforge-pg:/var/lib/postgresql/data
healthcheck:
...
/** Execution evidence captured on-device (photo/note/signature). _/
...
actorId: string;
/_* Free-form note or measurement value. _/
value?: string;
/_* SHA-256 of an optional captured media asset. */
assetHash?: string;
mimeType?: string;
}
...
@@ -36,8 +36,13 @@ services:
CORS_ORIGIN: http://localhost:1420
PORT: 8080 # Real AI provider (server-side only; never exposed to the browser).
