...
test('export a personal release from the editor', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Export personal release' }).click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toContain('.gforge');

  await expect(page.locator('.release-note')).toContainText('personal release', { timeout: 5000 });
});
