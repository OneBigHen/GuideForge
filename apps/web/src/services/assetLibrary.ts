...
export class AssetLibrary {
  ...
  /** True when these exact bytes are already in the content-addressed store. */
  async hasContent(bytes: Uint8Array): Promise<boolean> {
...
/**
 * Deterministically materialize every SEED_CATALOG procedural asset.
 *
 * Idempotent: generated bytes are content-addressed, so repeated runs
 * deduplicate in the store and no duplicate metadata rows are written.
 * Failures identify the template that failed. Provenance stays CC0/procedural;
 * nothing is downloaded from third parties.
 */
export async function ensureSeedCatalog(library: AssetLibrary): Promise<SeedCatalogResult> {
  let created = 0;
  let existing = 0;
  for (const seed of SEED_CATALOG) {
    try {
      const bytes = generateProceduralGlb(seed.template);
      if (await library.hasContent(bytes)) {
        existing += 1;
        continue;
      }
      await library.addProcedural(seed.template);
      created += 1;
    } catch (err) {
      throw new Error(
        `seed catalog entry "${seed.template}" failed: ${err instanceof Error ? err.message : String(err)}`,
        { cause: err },
      );
    }
  }
