...
describe('server-side Turnstile verification', () => {
  it('accepts a successful verification', async () => {
    const fetcher = vi.fn(
      (_url: unknown, init?: { body?: string }) => Promise.resolve(jsonResponse(OK_BODY)),
    );
  ...
  it('rejects hostname mismatch where configured', async () => {
    const fetcher = vi.fn(() => Promise.resolve(jsonResponse(OK_BODY)));
  ...
  it('rejects empty or oversized tokens before any network call', async () => {
    const fetcher = vi.fn();
    expect(
      (await verifyTurnstile('', undefined, { secretKey: 's', fetcher: fetcher as unknown as typeof fetch })).ok,
    ).toBe(false);
    expect(
