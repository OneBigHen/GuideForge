...
import {
  ...
  snapValue,
  subVec,
  worldPointForSurfaceAttachment,
  type SceneNode,
  type Transform,
} from './index.js';
...
describe('scene-core durable surface attachments', () => {
  it('keeps a mesh-local attachment stable through transforms and asset reuse', () => {
    let state = freshSceneState();
    state = applySceneCommand(
      state,
      cmd(SCENE_COMMAND_TYPES.addNode, { node: node(N1, 'Control') }),
    );
    const attachment = createGeometricSurfaceAttachment({
      attachmentId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' as EntityId,
      nodeId: N1,
      assetHash: null,
      localPoint: { x: 1, y: 0, z: 0 },
    });
    state = applySceneCommand(state, cmd(SCENE_COMMAND_TYPES.addSurfaceAttachment, { attachment }));
    const before = worldPointForSurfaceAttachment(state, attachment.attachmentId);
    state = applySceneCommand(
      state,
      cmd(SCENE_COMMAND_TYPES.setTransform, {
        nodeIds: [N1],
    ...
    expect(state.surfaceAttachments[0]?.localPoint).toEqual({ x: 1, y: 0, z: 0 });
    expect(before).toEqual({ x: 1, y: 0, z: 0 });
    expect(worldPointForSurfaceAttachment(state, attachment.attachmentId)).toEqual({
      x: 6,
      y: 0,
      z: 0,
    });
    ...
    state = applySceneCommand(
      state,
      cmd(SCENE_COMMAND_TYPES.setAsset, { nodeId: N1, assetHash: 'b'.repeat(64) }),
    );
    expect(state.surfaceAttachments[0]?.reviewState).toBe('needs-correction');
  });
});
